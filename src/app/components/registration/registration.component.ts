...
export class RegistrationComponent {
  ...
  public submit(): void {
    ...
    if (!regex.test(this.illumineerName)) {
      ...
      this.flashMessageService.setMessageText("Illumineer's name must be 3-20 characters: letters, numbers, underscores, no spaces or special characters.")
      return
    }
    ...
    if (!regex.test(this.username)) {
      ...
      this.flashMessageService.setMessageType("information");
      this.flashMessageService.setMessageText("Enter a valid email with letters, numbers, periods, and a domain.")
      return
    }
    ...
    if (!regex.test(this.password)) {
      ...
      this.flashMessageService.setMessageType("information");
      this.flashMessageService.setMessageText("Password must be at least 8 characters, with one lowercase, one uppercase, one number, and one special character.")
      return
    }

    if (this.password != this.confirmPassword) {
      this.flashMessageService.setMessageType("information");
      this.flashMessageService.setMessageText("Passwords must match.")
      return
    }
    ...
    this.registrationService.saveUserInBdd(this.illumineerName, this.username, this.password).subscribe({
      next: (response: Map<String, string>) => {
        ...
        if (response.has("success")) {
          ...
          this.flashMessageService.setMessageText(response.get("success")!);
          this.router.navigate(['']);
        } else {
          this.flashMessageService.setMessageType("error");
          this.flashMessageService.setMessageText(response.get("error")!);
        }
